...
/*   Updated: 2018/06/01 16:39:13 by yadouble         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_width_type_option(t_var *var)
{
	...
	{
		i = i + 2;
		if (var->check.prec & 1 && var->check.precwidth == 0 && var->check.nb != 0)
		{
			while (i++ < var->check.width - var->check.len)
				ft_buffer(var, ' ');
			i = -1;
		}
		else if (var->check.prec & 1 && var->check.precwidth == 0 && var->check.nb == 0)
			i = i - 2;
	}
